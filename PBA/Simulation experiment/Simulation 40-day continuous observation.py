"""
The script is designed for a simulation experiment: continuous observation for 40 days, totaling 20 hours.
It generates bursts using random numbers and performs periodic search using PBA.
Before running this script, please first calculate the FAP distribution for the continuous observation of 40 days and 20 hours
(using the script "Calculate the FAP distribution (Simulation experiment.)",
which requires the user to set the number of observation days) to calculate sigma.
...
def task(actual_period, actual_p, actual_pbp, actual_burst_rate):
    ...
    # Read the FAP distribution. The user reads the calculation result file based on the FAP distribution (Simulation experiment).
    p_cumu_dis = read_all_files('D:/PyCharm/Project/Project_FRB/模拟数据/PPBA/高精度周期检测/Pcumu分布/连续观测' + str(simulated_obs_day_num) + '天窗口和暴发率'+str(simulated_burst_rate)+'相宽'+str(simulated_p)+'/最小Pcumu数据')
