"""
This script is used to calculate the FAP distribution in the simulation experiment (continuous observation for 5, 10, 20, and 40 days).
It calculates the cumulative binomial probability distribution of PBA for non-periodic bursts within these observation windows.
A total of 10,000 tests are performed to derive the FAP distribution.
The user needs to adjust the "simulated_obs_day_num" variable in the task to
change the number of continuous observation days (5, 10, 20, or 40) and obtain the corresponding FAP distributions.
...
def task(actual_period, actual_p, actual_pbp, actual_burst_rate, jincheng):
    ...
    def period_search(test_start_d, test_end_d, obs_dis):
        ...
        """
        fap = np.zeros((nT, ncp, npw))  # Store the cumulative probability of the binomial distribution.
